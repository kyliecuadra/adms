                  ...
                  <div class="modal-content">
                      ...
                      <div class="modal-body">
                          ...
                          <table class="table" id="programsTable">
                              <thead>
                                  <tr>
                                      <th scope="col">List of Programs</th>
...
$(document).ready(function() {
  ...
  $(document).ready(function() {
    // Initialize DataTable for the accreditors
    $('#accreditors').DataTable({
        ajax: {
            url: 'display_faculty.php', // Ensure this is the correct URL
            dataSrc: 'data'
        },
        columns: [
            { data: 'name' },
    ...
    $('#accreditors tbody').on('click', '.set-role-btn', function() {
        ...
        // Load programs when the modal is opened
        loadPrograms();
    });

    // Function to load programs into the modal
    function loadPrograms() {
        $.ajax({
            url: 'display_programs.php', // Ensure this is the correct URL
            type: 'GET',
            dataType: 'json',
            success: function(programs) {
                let tbody = $('#programsTable tbody');
                tbody.empty(); // Clear existing rows

                // Append new rows for each program
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="toggle${index + 1}">
                                    <label class="form-check-label" for="toggle${index + 1}">Enable</label>
                                </div>
                            </td>
                            <td>
                                <div class="dropdown">
                                    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdown${index + 1}" data-bs-toggle="dropdown" aria-expanded="false">
                                        Select Area
                                    </button>
                                    <ul class="dropdown-menu" aria-labelledby="dropdown${index + 1}">
                                        ${Array.from({ length: 10 }, (_, i) => `<li><a class="dropdown-item" href="#">Area ${i + 1}</a></li>`).join('')}
                                    </ul>
                                </div>
                            </td>
                        </tr>
 ...
 $('#accreditors tbody').on('click', '.set-role-btn', function() {
    ...
    $(document).ready(function() {
  // Load programs when the modal is opened
  $('#setRoleModal').on('show.bs.modal', function() {
    $.ajax({
      url: 'display_programs.php',
      type: 'GET',
      dataType: 'json',
      success: function(programs) {
        let tbody = $('#programsTable tbody');
