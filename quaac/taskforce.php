...
<body>
 ...
 <div class="content-wrapper">
          <!-- Content -->
          <div class="container-xxl flex-grow-1 container-p-y">
            <!-- DISPLAY FACULTY WITH TASK AND PROGRAMS ASSIGN -->
            <div class="card p-4">
              <div class="mb-4">
              <a href="facultylist.php" class="btn w-15 btn-danger float-start mb-4 mx-4 text-white ">Faculty Member List</a>
              </div>
          ...
          <!-- Content wrapper -->
            ...
            <div class="modal fade" id="programModal" tabindex="-1" aria-labelledby="programModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="programModalLabel">Program Handle Details</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <!-- Program handle data will be inserted here -->
                            <p id="facultyName"></p>
...
$(document).ready(function() {
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    return `<button class="btn btn-info btn-sm program-btn" data-programhandle="${data}" data-name="${row.name}" data-bs-toggle="modal" data-bs-target="#programModal">
            ...
            { 
                data: 'task',
                render: function(data, type, row) {
                    return `<button class="btn btn-info btn-sm task-btn" data-task="${data}" data-name="${row.name}" data-email="${row.email}" data-bs-toggle="modal" data-bs-target="#taskModal">
    ...
    // Handle program modal display
    $('#accreditors').on('click', '.program-btn', function() {
        var facultyName = $(this).data('name');
        var programData = $(this).data('programhandle'); 
        $('#programModal').modal('show');
        $('#programModal').find('#facultyName').text('Faculty Name: ' + facultyName);
        $('#programModal').find('#programDetails').text('Program Handle: ' + programData);
    });

    // Handle task modal display
    $('#accreditors').on('click', '.task-btn', function() {
        var taskData = $(this).data('task');
        var name = $(this).data('name');
        var email = $(this).data('email');
        
        $('#taskModal').modal('show').find('.modal-body').html(`
            <p><strong>Name:</strong> ${name}</p>
            <p><strong>Email:</strong> ${email}</p>
            
            <div class="form-group">
                <label for="dropdown1">Document Area:</label>
                <select id="dropdown1" class="form-control">
                <option value="Option1">Choose Document Area</option>
                    <option value="Option1">Option 1</option>
                    <option value="Option2">Option 2</option>
                </select>
            </div>
            <div class="form-group">
                <label for="dropdown2">Programs for College of Engineering and Information Technology:</label>
                <select id="dropdown2" class="form-control">
                    <option value="Option1">Choose Program</option>
                    <option value="Option1">Option 1</option>
                    <option value="Option2">Option 2</option>
                </select>
            </div>
